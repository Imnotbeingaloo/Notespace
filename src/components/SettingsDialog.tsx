...
import { User as UserIcon, SlidersHorizontal, Palette, Database, Loader2, Sun, Moon, Monitor, Download, Trash2, Check, Lock, BookOpen, Clock, Target, ChevronRight, ChevronLeft, Paperclip } from "lucide-react";
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-3xl w-screen h-[100dvh] max-w-none sm:w-[95vw] sm:h-auto sm:rounded-lg rounded-none p-0 overflow-hidden gap-0 border-0 sm:border translate-x-0 translate-y-0 left-0 top-0 sm:left-[50%] sm:top-[50%] sm:translate-x-[-50%] sm:translate-y-[-50%] duration-200 data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:slide-out-to-bottom sm:data-[state=closed]:slide-out-to-bottom-0 data-[state=open]:slide-in-from-bottom sm:data-[state=open]:slide-in-from-bottom-0 sm:data-[state=closed]:zoom-out-95 sm:data-[state=open]:zoom-in-95">
        {/* Mobile top bar with Done */}
        <div className="sm:hidden flex items-center justify-between px-4 h-12 border-b border-border shrink-0">
          <h2 className="font-serif text-base font-bold text-foreground">Settings</h2>
          <button
        ...
        <div className="flex flex-row sm:min-h-[480px] min-w-0 w-full flex-1 min-h-0 sm:flex-none">
          {/* Side tabs — compact on mobile, full on desktop */}
          <aside className="w-[96px] sm:w-52 shrink-0 bg-muted/30 border-r border-border p-2 sm:p-3 min-w-0 overflow-y-auto">
            <h2 className="hidden sm:block px-2 py-2 text-xs uppercase tracking-wider font-mono text-muted-foreground">Settings</h2>
            <nav className="flex flex-col gap-1">
              {TABS.map(({ id, label, Icon }) => (
                <button
                  key={id}
                  onClick={() => setTab(id)}
                  className={`flex flex-col sm:flex-row items-center sm:gap-2 gap-1 px-2 sm:px-3 py-2.5 sm:py-2 text-[11px] sm:text-sm rounded-lg transition-all whitespace-nowrap text-center sm:text-left ${
                    tab === id
          ...
          <section className="flex-1 min-w-0 overflow-y-auto sm:max-h-none sm:p-8">
            <div key={tab} className="p-4 sm:p-0 animate-fade-in">
            {tab === "personal" && (
