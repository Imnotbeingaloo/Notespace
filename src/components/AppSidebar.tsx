...
import { Plus, BookOpen, Trash2, ChevronRight, ChevronDown, Menu, FileText, NotebookPen, LogOut, Upload, Home, Pencil, Search as SearchIcon, RotateCcw, Tag, CalendarDays, X, Settings, MoreHorizontal } from "lucide-react";
    ...
    >
      ...
      {!collapsed && (
        <div className="px-2 space-y-1 mb-2 border-t border-sidebar-border pt-2">
          ...
          {/* Study Schedule - always collapsible */}
          <button
            ...
            {scheduleOpen && (
              ...
              >
                <div className="px-1 pb-2 pt-1 space-y-1">
                  {upcomingPlans.length === 0 ? (
                    <p className="text-[11px] text-muted-foreground px-2 py-1">🎉 No upcoming sessions. Open the Study Planner to create one!</p>
                  ) : (
                  ...
                  >
                    <Plus className="h-3 w-3" />
                    {upcomingPlans.length === 0 ? "Open Study Planner" : "Create new session"}
                  </button>
                </div>
              </motion.div>
            )}
